...
bool _TF_FN TF_RemoveIdListener(TinyFrame *tf, TF_ID frame_id)
{
    ...
    TF_Error("ID listener %d to remove not found", (int32)frame_id);
    return false;
}
...
bool _TF_FN TF_RemoveTypeListener(TinyFrame *tf, TF_TYPE type)
{
    ...
    TF_Error("Type listener %d to remove not found", (int32)type);
    return false;
}
...
//    TF_Error("Unhandled message, type %d", (int32)msg.type);
//}
...
bool _TF_FN TF_RenewIdListener(TinyFrame *tf, TF_ID id)
{
    ...
    TF_Error("Renew listener: not found (id %d)", (int32)id);
    return false;
}
...
char _TF_FN TF_Accept(TinyFrame *tf, const uint8_t *buffer, uint32_t count)
{
    uint32_t i;
    for (i = 0; i < count; i++) {
...
char _TF_FN TF_AcceptChar(TinyFrame *tf, unsigned char c)// fixed by cxy  20190521
{
    int32 result =0;
    {
        ...
        case TFState_HEAD_CKSUM:
            COLLECT_NUMBER(tf->ref_cksum, TF_CKSUM) {
                ...
                if (tf->len > TF_MAX_PAYLOAD_RX) {
                    TF_Error("Rx payload too long: %d", (int32)tf->len);
...
// use variables: si - signed int32, b - byte, outbuff - target buffer, pos - count of bytes in buffer
...
{
    int8_t si = 0; // signed small int32
    uint8_t b = 0;
    TF_ID id = 0;
    TF_CKSUM cksum = 0;
    uint32_t pos = 0;
...
void _TF_FN TF_Tick(TinyFrame *tf)
{
    ...
    for (i = 0; i < tf->count_id_lst; i++) {
        ...
        // count down...
        if (--lst->timeout == 0) {
            TF_Error("ID listener %d has expired", (int32)lst->id);
...
void TF_WriteImpl(TinyFrame *tf, const uint8_t *buff, uint32_t len)
{
    ...
    {
        UART0->DATA = *(buff+count);
        //circulates until the transmission is complete
         while((UART0->STATE&0X01)== 0x01);
    }
}
uint8 g_TfRxBuff[15];
volatile uint32 g_TfRxCount;
/****************************************************************************************
*Function name : tinyFramefRx tfRxExample
*Description   : uart receive data through tinyframe protocol
*Paramater	   :
*        @ :  
*        @ :   
*Paramater	selection:		
...
TinyFrameRx tinyFramefRx(void)
{
    int32 tempdata;
    TinyFrameRx revData;
    TinyFrame * ft_tmp = TF_Init(TF_SLAVE);
    if(ft_tmp == NULL)
	{
		TinyFrame * ft_tmp = TF_Init(TF_SLAVE);
	}
	...
	{
        while(g_TfRxCount!=15){}
        g_TfRxCount = 0;
        while (0 != TF_Accept(ft_tmp, g_TfRxBuff, sizeof(g_TfRxBuff))){}
        TF_DeInit(ft_tmp);
    }				
    switch(ft_tmp->type)
    {
        case 0x0100:
            tempdata = (ft_tmp->data[0]+ft_tmp->data[1]*256+ft_tmp->data[2]*256*256+ft_tmp->data[3]*256*256*256);
            revData.revfloatData = *(float*)&tempdata;
        case 0x0101:
            tempdata = (ft_tmp->data[0]+ft_tmp->data[1]*256+ft_tmp->data[2]*256*256+ft_tmp->data[3]*256*256*256);
            revData.revuint32Data = (int32)tempdata;
        case 0x0102:
            tempdata = (ft_tmp->data[0]+ft_tmp->data[1]*256+ft_tmp->data[2]*256*256+ft_tmp->data[3]*256*256*256);
            revData.revuint32Data = (int32)tempdata;
        case 0x0103:
    ...
    revData.receiveType = ft_tmp->type;   
    memset(g_TfRxBuff, 0 , sizeof(g_TfRxBuff));
    return revData;
    //printf("TinyFrame Rx\r\n");
}

void tfRxExample(void)
{
    TinyFrame * ft_tmp = TF_Init(TF_SLAVE);
    if(ft_tmp == NULL)
	{
...
*Function name : tinyFramefTx tfTxExample
*Description   : uart send data through tinyframe protocol
*Paramater	   :
*        @type : send data type
*        @data : send data address
*        @len  : send data len
*Paramater	selection:		
...
void tinyFramefTx(TF_TYPE type, uint8 *data, TF_LEN len)
{
    TF_Msg  msg_tmp;
    msg_tmp.len = len;
    TinyFrame * ft_tmp =NULL;
    ft_tmp = TF_Init(TF_SLAVE);
    if(ft_tmp == NULL)
    {
        ft_tmp = TF_Init(TF_SLAVE);
    }
    ...
    {
        TF_SendSimple(ft_tmp, type, data, msg_tmp.len);
        TF_DeInit(ft_tmp);
    }
}
void tfTxExample(void)
{
    //uint16  num = 0;
    ////uint8 tempBuff[4]={0x12, 0x34, 0x56, 0x78};
    //float tempFbfff[1]={3.14};
    //TF_Msg  msg_tmp;
    //multTargetInfo myMultTargetInfo;
    //targetState myTargetState;
    ////Example---0x0A00 0x0A01
    ////Store data
    //for (int i=0; i<TARGET_NUM; i++)
    //{
    //    myMultTargetInfo.multTarget[num].range    = 1.0;
    //    myMultTargetInfo.multTarget[num].velocity = 2.0;
    //    myMultTargetInfo.multTarget[num].power    = 3.0;
    //    myMultTargetInfo.multTarget[num].phase0   = 4.0;
    //    myMultTargetInfo.multTarget[num].phase1   = 5.0;
    //    myMultTargetInfo.multTarget[num].angleH   = 6.0;
    //    myMultTargetInfo.multTarget[num].angleV   = 7.0;
    //    num++;
    //}
    //myMultTargetInfo.targetNum = num;
    //
    ////Example---0x0A00 
    ////msg_tmp.len = 4+num*28;
    //////Example---0x0A01 
    //////msg_tmp.len = 4+num*16;    
    ////
    ////
    //TinyFrame * ft_tmp =NULL;
    ////ft_tmp = TF_Init(TF_SLAVE);
    ...
    ////} else
    ////{
    ////    TF_SendSimple(ft_tmp,0x0A00,(uint8*)&myMultTargetInfo,msg_tmp.len);
    ////    TF_DeInit(ft_tmp);
    ////}
    //
    //tinyFramefTx(0x0A00, (uint8*)&myMultTargetInfo, 4+num*28);
    //
    //
    ////Example---0x0B00 0x0B01 0x0Cxx 0x0Dxx
    ////1 chirp adc data 512x2(IQ) 
    ////start address is 0x20008000   
    //
    //
    //msg_tmp.len = 512*2;
    //ft_tmp = TF_Init(TF_SLAVE);
    ...
    //} else
    //{
    //    TF_SendSimple(ft_tmp,0x0B00,(uint8*)(0x20008000),msg_tmp.len);
    //    TF_DeInit(ft_tmp);
    //}
    //
    ////Example---0x0E00
    //myTargetState.targetFound = 0x0055;
    //myTargetState.targetLose  = 0x0066;
    //msg_tmp.len = 2;
    //ft_tmp = TF_Init(TF_SLAVE);
    ////target found
    //if(ft_tmp == NULL)
    ...
    //} else
    //{
    //    TF_SendSimple(ft_tmp,0x0D00,(uint8*)(&myTargetState.targetFound),msg_tmp.len);
    //    TF_DeInit(ft_tmp);
    //}
    //
    ////target found
    ...
    //} else
    //{
    //    TF_SendSimple(ft_tmp,0x0D00,(uint8*)(&myTargetState.targetLose),msg_tmp.len);
    //    TF_DeInit(ft_tmp);
    //}
    //
    ////generate tx data
    //msg_tmp.len = 4;
    //ft_tmp = TF_Init(TF_MASTER);
    //if(ft_tmp == NULL)
    //{
    //    ft_tmp = TF_Init(TF_MASTER);
    //}
    ...
    //} else
    //{
    //    TF_SendSimple(ft_tmp,0x0100,(uint8*)(&tempFbfff),msg_tmp.len);
    //    TF_DeInit(ft_tmp);
    //}
}
/****************************************************************************************
*Function name : printfTf 
*Description   : uart sends printing information to GUI through tinyframe protocol
*Paramater	   :
*        @ :  
*        @ :   
*Paramater	selection:
...
int16 printfTf(const char *format, ...)
{
    ...
    va_list vl;
    va_start(vl, format);
    int16 ret = vsprintf(bufferTemp, format, vl);
    va_end(vl);
    tinyFramefTx(0x0100, (uint8*)&bufferTemp, ret);
    return ret;
}
