...
/****************************************************************************************
*Function name : pllnLock
*Description   : plln lock 
...
float vtuneRead(void)
{
    ...
    volatile uint8 gpadcValue;
    RFC->rf_plln_regc_reg |= 0x72300000;
    gpadcEnable(GPADC_IN18_2);
    //After testing, we need to read gpadc data many times.
    delayUs(3000, HIGH_CLOCK);
    gpadcValue = *(uint8*)0x400030d4;
    delayUs(3000, HIGH_CLOCK);
    gpadcValue = *(uint8*)0x400030d4;
    delayUs(3000, HIGH_CLOCK);
    gpadcValue = *(uint8*)0x400030d4;
    
    vtuneValue = (float)(((float)(gpadcValue/255.0))*1.4);
    RFC->rf_plln_regc_reg &= ~(0xfff00000);
    return vtuneValue;
}
void pllnLock(float fstart, uint32 fm)
{
    uint32 pllnLockStatus;
    uint8 vcoCap = 0;
    int   vcoSwNumStart = 0;
    int   vcoSwNum = 0;
    int32 fstartFlag = 0;
    ...
    regPllnRegb = RFC->rf_plln_regb_reg;
    pllnLockStatus = (*(uint32*)0x40003194)&(1<<2);
    if(pllnLockStatus == 4)
        fstartFlag = 1;
    else
        fstartFlag = 0;
    //second---.In vco_sw[0:15], fine the value that the start frquency and end
    //frequency are loceked.
    goto vcoSwitch;
    ...
    {   
        ...
        vtuneValid = 0; 
        vcoSwNum = 0;       
        memset(vcoSwValid, 0 , sizeof(vcoSwValid));        
        while(1)
        {
            vcoCap++;
            if(vcoCap == 16) {break;}
            
            RFC->rf_plln_regb_reg = regPllnRegb | (vcoCap<<8);            
            
            fmcwConfig(fstart, 0, CHIRP_T0, CHIRP_T1, CHIRP_T2, CHIRP_NUM);
            fmcwEnmode(REGISTER, CONTINUE_MODE);
            delayUs(1000,HIGH_CLOCK);

            pllnLockStatus = (RFC->rf_plln_rege_reg)&(1<<2);
            ...
            if(fm != 0)
            {
                fmcwConfig(fstart+(fm/1000), 0, CHIRP_T0, CHIRP_T1, CHIRP_T2, CHIRP_NUM);
                fmcwEnmode(REGISTER, CONTINUE_MODE);
                delayUs(1000,HIGH_CLOCK);
            ...
            if ((fstartFlag == 1) && (fendtFlag == 1))
            {
                vcoSwValid[vcoCap] = vcoCap;
            }
        }
        for(int vcoIndex=0; vcoIndex<16; vcoIndex++)
        {
            if(vcoSwValid[vcoIndex] != 0)
            {   
                //printf("valid vco index is : %d\r\n",vcoIndex);
                if(vcoSwNum == 0)
                {
                    vcoSwNumStart = vcoIndex;
                }
                vcoSwNum ++;
            }
        }
    ...
    if (vcoSwNum != 0)
    {
        // if there are two valid vco_sw, choose the larger one.  XX for pcb803. 
        //if ((vcoSwNum%2)==0) vcoSwNumStart--;

        RFC->rf_plln_regb_reg = regPllnRegb | (((int)(vcoSwNumStart+vcoSwNum/2))<<8);          
    }else{
        fstartFlag = 0;
        fendtFlag  = 0;
        vcoCap = 0;
