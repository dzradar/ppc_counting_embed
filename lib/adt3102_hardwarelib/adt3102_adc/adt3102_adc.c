...
/****************************************************************************************
*Function name : adcDataCorrection
*Description   : correction of ADC sampling problem			
			Matlab code: if((floor(data_tmp/64)>31) | (floor(data_tmp/64)<31)) 
											data_tmp = floor(data_tmp/64)64+mod(data_tmp,64)1.75; end
*Paramater	   :
*        @addr  :Destination address
*        @adcSamples :ADC sampling points
*        @chirpNum     :chirp number
*Return        : None
****************************************************************************************/
void	adcDataCorrection(uint32 addr,uint16 adcSamples,uint16 chirpNum)
{
	...
	int16 data_tmp;
	uint16 temp = adcSamples*chirpNum;
    int16 dataHigh;
    int16 dataLow;
	for(int i=0;i<=temp;i++)
	{
		data_tmp = *(address+i);
        dataHigh=data_tmp&0xfc00;
        dataLow=data_tmp&0x03ff;
		if(dataLow<0x3FF) //for FIR coef16=0xfff instead of 1024
		{
			data_tmp = dataHigh + (dataLow<<1);
		  *(address+i) = data_tmp;
		}
	}
}
